...
//                                                    //
//             Object Positioning System V1.3         //
////////////////////////////////////////////////////////
...
Lx_fnc_copyObject = {
	...
	if (!isNull lxObj) then {
		...
		_txtType = "Code copied in clipboard";
		if (_type == 1) then {
			_txtType = "Code copied in RPT";
		};
...
KK_fnc_floatToString = { private "_arr"; _arr = toArray str (_this % 1); _arr set [0, 'x']; _arr = _arr - ['x']; toString (toArray str (_this - _this % 1) + _arr) };

KK_fnc_positionToString = {
	{ _this = if (_forEachIndex == 0) then [ {_x call KK_fnc_floatToString}, {_this + "," + (_x call KK_fnc_floatToString)} ]; } forEach +_this; _this
