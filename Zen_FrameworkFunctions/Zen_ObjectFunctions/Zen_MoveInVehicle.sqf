...
switch (toLower _vehicleSlot) do {
    case "cargo": {
        ...
        _args = [_unitsArray, _vehicle, _turrets];
        ZEN_FMW_MP_REAll("Zen_MoveInVehicle_Turret_MP", _args)

        WAITUNTIL_MP_MOVE_SYNCH_OBJ_VAR(_unitsArray)
        _unitsArray = [_unitsArray, {(vehicle _this != _this)}] call Zen_ArrayFilterCondition;
        if (count _unitsArray > 0) then {

            _args = [_unitsArray, _vehicle];
            ZEN_FMW_MP_REAll("Zen_MoveInVehicle_Cargo_MP", _args)
        };
    };
    case "driver": {
        ...
        ZEN_FMW_MP_REAll("Zen_MoveInVehicle_Driver_MP", _args)
    };
    case "turret": {
